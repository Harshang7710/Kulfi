...
app.get('/manager/pos', requireRole('manager'), aw(async (req, res) => {
  ...
  const body = `<form method="post" action="/manager/pos" class="kulfi-pos counter-pos pos-billing" data-pos-form><div class="counter-header"><div><p class="eyebrow">Franchise counter</p><h2>Kulfi Cart</h2><p class="muted">Keep multiple customer bills ready while stock and payments stay accurate.</p></div><div class="counter-operator"><span>Manager</span><strong>${esc(req.user.name)}</strong></div></div><section class="counter-shell"><div class="counter-workspace"><div class="filter-panel"><label>Find kulfi<input data-product-search placeholder="Search by item name or code"></label><label>Stock view<select data-stock-filter><option value="all">All active items</option><option value="in">Ready in Main Fridge</option><option value="low">Low / no stock</option></select></label><button class="btn secondary" type="button" data-product-reset>Reset</button></div><div class="product-board">${rows.map(r => { const display = stockDisplay(r); const inStock = r.mainFridgeQty > 0; return `<article class="item-row product-card ${r.mainFridgeQty <= r.lowStockThreshold ? 'low' : ''}" data-price="${Number(r.mrp || 0)}" data-product-name="${esc(`${r.name} ${r.itemCode || ''}`).toLowerCase()}" data-stock-status="${inStock ? 'in' : 'low'}"><div class="product-media">${r.imageData ? `<img class="item-thumb" src="${esc(r.imageData)}" alt="${esc(r.name)} image">` : '<span>🍦</span>'}</div><div class="product-info"><h3>${esc(r.name)}</h3><p>₹${money(r.mrp)}</p><span class="badge ${inStock ? 'ok' : 'danger'}">${inStock ? 'In stock' : 'Refill needed'}</span></div><div class="product-stock"><span>Main Fridge: <strong>${display.mainPieces} pcs</strong></span><span>Second Fridge: <strong>${display.secondBoxes} boxes</strong></span></div><div class="qty-controls"><button class="qty-btn" type="button" data-qty-step="-1">−</button><input class="sale-qty" name="qty_${r.id}" type="number" min="0" max="${r.mainFridgeQty}" value="0" inputmode="numeric"><button class="btn primary add-btn" type="button" data-qty-step="1">+ Add</button><label class="inline-check"><input class="free-toggle" type="checkbox" name="free_${r.id}" value="1"> Complimentary</label></div><output class="line-total">₹0.00</output><div class="transfer-controls"><input name="transfer_${r.id}" type="number" min="0" max="${r.secondFridgeQty}" placeholder="Move boxes"><button formaction="/manager/transfer" name="itemId" value="${r.id}" class="btn secondary">Transfer</button></div></article>`; }).join('') || '<p class="empty">No active items are available.</p>'}</div><div class="bottom-drafts"><span class="draft-label">Draft bills</span>${billTabs}<select data-draft-select aria-label="Draft bill selector"></select><button class="draft-action" type="button" data-draft-save>Save Draft</button><button class="draft-action" type="button" data-draft-new>New Draft</button><button class="draft-action danger-text" type="button" data-draft-delete>Delete</button></div></div><aside class="card payment-card pos-cart"><h2>Cart</h2><div class="cart-preview" data-cart-preview><p class="empty">No items added yet.</p></div><div class="cart-total-row"><strong>Total</strong><strong data-cart-total>₹0.00</strong></div><label>Customer Name<input name="customerName" data-draft-field placeholder="Customer name"></label><div class="payment-split"><label>Cash<input name="cashAmount" data-cash-amount type="number" min="0" step="0.01" value="0.00"></label><label>Online<input name="onlineAmount" data-online-amount type="number" min="0" step="0.01" value="0.00"></label></div><input name="totalAmountPreview" data-total-amount type="hidden" value="0.00"><div class="payment-actions"><button class="btn secondary" type="button" data-pay-mode="cash">All Cash</button><button class="btn secondary" type="button" data-pay-mode="online">All Online</button></div><label>Remarks<textarea name="remark" rows="3" data-draft-field placeholder="Global bill remarks"></textarea></label><button class="primary save-bill" data-finalize-bill>Save Bill</button></aside></section><div class="mobile-cart-bar"><span>🛒 <strong data-mobile-cart-count>0 items</strong><small>Total</small></span><strong data-mobile-cart-total>₹0.00</strong></div></form>`;
