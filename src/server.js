...
app.get('/manager/pos', requireRole('manager'), aw(async (req, res) => {
  ...
  const body = `<form method="post" action="/manager/pos" class="kulfi-pos wireframe-pos pos-billing" data-pos-form><div class="pos-navbar">Navbar</div><section class="pos-main-area"><div class="pos-left"><div class="filter-panel"><label>Find kulfi<input data-product-search placeholder="Search by item name or code"></label><label>Stock view<select data-stock-filter><option value="all">All active items</option><option value="in">In stock</option><option value="low">Low / no stock</option></select></label><button class="btn secondary" type="button" data-product-reset>Reset</button></div><div class="product-board">${rows.map(r => { const display = stockDisplay(r); const inStock = r.mainFridgeQty > 0; return `<article class="item-row product-card ${r.mainFridgeQty <= r.lowStockThreshold ? 'low' : ''}" data-price="${Number(r.mrp || 0)}" data-product-name="${esc(`${r.name} ${r.itemCode || ''}`).toLowerCase()}" data-stock-status="${inStock ? 'in' : 'low'}"><div class="product-media">${r.imageData ? `<img class="item-thumb" src="${esc(r.imageData)}" alt="${esc(r.name)} image">` : '<span>Photo</span>'}</div><div class="product-info"><h3>${esc(r.name)}</h3><p>₹${money(r.mrp)}</p></div><div class="product-stock"><strong>Stock</strong><span>Main: ${display.mainPieces} pcs</span><span>Second: ${display.secondBoxes} boxes</span></div><div class="qty-controls"><button class="qty-btn" type="button" data-qty-step="-1">−</button><input class="sale-qty" name="qty_${r.id}" type="number" min="0" max="${r.mainFridgeQty}" value="0" inputmode="numeric"><button class="btn primary add-btn" type="button" data-qty-step="1">Add Button</button><label class="inline-check"><input class="free-toggle" type="checkbox" name="free_${r.id}" value="1"> Free Check Box</label></div><output class="line-total">₹0.00</output><div class="transfer-controls"><input name="transfer_${r.id}" type="number" min="0" max="${r.secondFridgeQty}" placeholder="Boxes"><button formaction="/manager/transfer" name="itemId" value="${r.id}" class="btn secondary">Transfer</button></div></article>`; }).join('') || '<p class="empty">No active items are available.</p>'}</div><div class="bottom-drafts">${billTabs}<select data-draft-select aria-label="Draft bill selector"></select><button class="draft-action" type="button" data-draft-save>Save a Draft</button><button class="draft-action" type="button" data-draft-new>New Draft</button><button class="draft-action" type="button" data-draft-delete>Delete</button></div></div><aside class="card payment-card pos-cart"><h2>Cart</h2><div class="cart-preview" data-cart-preview><p class="empty">No items added yet.</p></div><div class="cart-total-row"><strong>Total</strong><strong data-cart-total>₹0.00</strong></div><label>Customer Name<input name="customerName" data-draft-field placeholder="Customer name"></label><label>Cash<input name="cashAmount" data-cash-amount type="number" min="0" step="0.01" value="0.00"></label><label>Online<input name="onlineAmount" data-online-amount type="number" min="0" step="0.01" value="0.00"></label><input name="totalAmountPreview" data-total-amount type="hidden" value="0.00"><div class="payment-actions"><button class="btn secondary" type="button" data-pay-mode="cash">Pay Cash</button><button class="btn secondary" type="button" data-pay-mode="online">Pay Online</button></div><label>Remarks<textarea name="remark" rows="3" data-draft-field></textarea></label><button class="primary save-bill" data-finalize-bill>Save Bill</button></aside></section><div class="mobile-cart-bar"><span>🛒 <strong data-mobile-cart-count>0 items</strong><small>Total</small></span><strong data-mobile-cart-total>₹0.00</strong></div></form>`;
