...
app.get('/manager/pos', requireRole('manager'), aw(async (req, res) => {
  ...
  const body = `<form method="post" action="/manager/pos" class="zepto-pos pos-billing" data-pos-form><header class="pos-storefront"><div><p class="eyebrow">Franchise counter</p><h2>Kulfi Cart</h2><p class="muted">Fast billing for walk-in kulfi orders with draft bills parked at the bottom.</p></div><label class="pos-search">🔎<input data-product-search placeholder="Search by kulfi name or item code"></label><div class="counter-operator"><span>Manager</span><strong>${esc(req.user.name)}</strong></div></header><section class="pos-market"><aside class="category-rail"><strong>Kulfi Types</strong><a class="active">Matka Kulfi</a><a>Stick Kulfi</a><a>Pops</a><a>Cups</a><a>Family Packs</a></aside><main class="market-body"><div class="filter-panel"><label>Stock view<select data-stock-filter><option value="all">All active items</option><option value="in">Main Fridge available</option><option value="low">Refill needed</option></select></label><button class="btn secondary" type="button" data-product-reset>Reset</button></div><div class="product-board">${rows.map(r => { const display = stockDisplay(r); const inStock = r.mainFridgeQty > 0; return `<article class="item-row product-card ${r.mainFridgeQty <= r.lowStockThreshold ? 'low' : ''}" data-price="${Number(r.mrp || 0)}" data-product-name="${esc(`${r.name} ${r.itemCode || ''}`).toLowerCase()}" data-stock-status="${inStock ? 'in' : 'low'}"><div class="product-media">${r.imageData ? `<img class="item-thumb" src="${esc(r.imageData)}" alt="${esc(r.name)} image">` : '<span>🍦</span>'}<button class="btn primary add-btn" type="button" data-qty-step="1">ADD</button></div><div class="product-info"><h3>${esc(r.name)}</h3><p class="price-line"><strong>₹${money(r.mrp)}</strong><span>${r.piecesPerBox} pcs/box</span></p><p class="stock-line">Main Fridge: <strong>${display.mainPieces} pcs</strong></p><p class="stock-line">Second Fridge: <strong>${display.secondBoxes} boxes</strong></p></div><div class="qty-controls"><button class="qty-btn" type="button" data-qty-step="-1">−</button><input class="sale-qty" name="qty_${r.id}" type="number" min="0" max="${r.mainFridgeQty}" value="0" inputmode="numeric"><label class="inline-check"><input class="free-toggle" type="checkbox" name="free_${r.id}" value="1"> Complimentary</label></div><output class="line-total">₹0.00</output></article>`; }).join('') || '<p class="empty">No active items are available.</p>'}</div></main><aside class="card payment-card pos-cart"><h2>Cart</h2><div class="cart-preview" data-cart-preview><p class="empty">No items added yet.</p></div><div class="cart-total-row"><strong>Total</strong><strong data-cart-total>₹0.00</strong></div><label>Customer Name<input name="customerName" data-draft-field placeholder="Customer name"></label><div class="payment-split"><label>Cash<input name="cashAmount" data-cash-amount type="number" min="0" step="0.01" value="0.00"></label><label>Online<input name="onlineAmount" data-online-amount type="number" min="0" step="0.01" value="0.00"></label></div><input name="totalAmountPreview" data-total-amount type="hidden" value="0.00"><div class="payment-actions"><button class="btn secondary" type="button" data-pay-mode="cash">All Cash</button><button class="btn secondary" type="button" data-pay-mode="online">All Online</button></div><label>Remarks<textarea name="remark" rows="3" data-draft-field placeholder="Global bill remarks"></textarea></label><button class="primary save-bill" data-finalize-bill>Save Bill</button></aside></section><div class="draft-dock"><span class="draft-label">Draft bills</span>${billTabs}<button class="draft-clear" type="button" data-draft-delete>Clear</button></div><div class="mobile-cart-bar"><span>🛒 <strong data-mobile-cart-count>0 items</strong><small>Total</small></span><strong data-mobile-cart-total>₹0.00</strong></div></form>`;
