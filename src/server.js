...
app.get('/manager/pos', requireRole('manager'), aw(async (req, res) => {
  ...
  const body = `<form method="post" action="/manager/pos" class="kulfi-pos pos-billing" data-pos-form><section class="card pos-catalog"><div class="kulfi-pos-top"><div><h2>Kulfi Cart</h2><p class="muted">Parallel billing for walk-in customers</p></div><div class="pos-top-icons"><span title="Draft help">ⓘ</span><span class="avatar-dot" title="${esc(req.user.name)}">👤</span></div></div><div class="bill-tabs">${billTabs}<select data-draft-select aria-label="Draft bill selector"></select><button class="btn secondary" type="button" data-draft-save>Save draft</button><button class="btn secondary" type="button" data-draft-new>New draft</button><button class="btn danger" type="button" data-draft-delete>Delete</button></div><div class="catalog-tools"><label class="search-field"><span>⌕</span><input data-product-search placeholder="Search kulfi"></label><select data-stock-filter><option value="all">All stock</option><option value="in">In stock</option><option value="low">Low stock</option></select><select aria-label="Category"><option>All categories</option></select></div><div class="product-grid">${rows.map(r => { const display = stockDisplay(r); const inStock = r.mainFridgeQty > 0; return `<article class="item-row product-card ${r.mainFridgeQty <= r.lowStockThreshold ? 'low' : ''}" data-price="${Number(r.mrp || 0)}" data-product-name="${esc(r.name).toLowerCase()}" data-stock-status="${inStock ? 'in' : 'low'}"><div class="product-media">${r.imageData ? `<img class="item-thumb" src="${esc(r.imageData)}" alt="${esc(r.name)} image">` : '<span>🍦</span>'}</div><div class="product-info"><h3>${esc(r.name)}</h3><p>₹${money(r.mrp)}</p><span class="badge ${inStock ? 'ok' : 'danger'}">${inStock ? 'In stock' : 'No stock'}</span></div><div class="product-stock"><span>Main: ${display.mainPieces} pcs</span><span>Second: ${display.secondBoxes} boxes</span><span>${r.piecesPerBox} pcs/box</span></div><div class="qty-controls"><button class="qty-btn" type="button" data-qty-step="-1">−</button><input class="sale-qty" name="qty_${r.id}" type="number" min="0" max="${r.mainFridgeQty}" value="0" inputmode="numeric"><button class="btn primary add-btn" type="button" data-qty-step="1">+ Add</button><label class="inline-check"><input class="free-toggle" type="checkbox" name="free_${r.id}" value="1"> Free</label></div><output class="line-total">₹0.00</output><div class="transfer-controls"><input name="transfer_${r.id}" type="number" min="0" max="${r.secondFridgeQty}" placeholder="Boxes"><button formaction="/manager/transfer" name="itemId" value="${r.id}" class="btn secondary">Transfer</button></div></article>`; }).join('') || '<p class="empty">No active items are available.</p>'}</div></section><aside class="card payment-card pos-cart"><h2>Cart</h2><div class="cart-preview" data-cart-preview><p class="empty">No items added yet.</p></div><div class="cart-total-row"><strong>Total</strong><strong data-cart-total>₹0.00</strong></div><label>Customer Name<input name="customerName" data-draft-field placeholder="Customer name"></label><label>Cash Amount<input name="cashAmount" data-cash-amount type="number" min="0" step="0.01" value="0.00"></label><label>Online Amount<input name="onlineAmount" data-online-amount type="number" min="0" step="0.01" value="0.00"></label><input name="totalAmountPreview" data-total-amount type="hidden" value="0.00"><div class="payment-actions"><button class="btn secondary" type="button" data-pay-mode="cash">Pay Cash</button><button class="btn secondary" type="button" data-pay-mode="online">Pay Online</button></div><label>Bill remark<textarea name="remark" rows="3" data-draft-field></textarea></label><button class="primary save-bill" data-finalize-bill>Save Bill</button></aside><div class="mobile-cart-bar"><span>🛒 <strong data-mobile-cart-count>0 items</strong><small>Total</small></span><strong data-mobile-cart-total>₹0.00</strong></div></form>`;
